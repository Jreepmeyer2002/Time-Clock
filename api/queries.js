...
module.exports = {
  getUser: function(request, response) {
    if(debug) {
      ...
      pool.query('SELECT * FROM person WHERE pid=$1', [id], (error, results) => {
      if (error) {
        throw error
      }
  ...
  getShifts: function(request, response){
    const id = parseInt(request.params.id);
    pool.query('SELECT timeLog.clockOut-timeLog.clockIn AS total, timeLog.clockOut, timeLog.date, timeLog.clockIn, timeLog.clockOut, person.name AS name FROM timeLog INNER JOIN person ON personid=person.pid WHERE personid=$1', [id], (error, results) => {
    if (error) {
      throw error
    }
    response.status(200).json(results.rows)
  ...
  clockIn: function(request,response) {
    const id = parseInt(request.params.id);
    pool.query('SELECT timeLog.end_time-timeLog.start_time AS total, timeLog.id AS shift_id, shifts.employee_id, shifts.weekday, shifts.start_time, shifts.end_time, employees.name AS name FROM shifts INNER JOIN employees ON employee_id=employees.id WHERE employee_id=$1', [id], (error, results) => {
        if (error) {
          throw error
        }
